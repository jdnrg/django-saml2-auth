...
def _get_metadata():
    if "METADATA_LOCAL_FILE_PATH" in settings.SAML2_AUTH:
        ...
        return {"remote": [{"url": settings.SAML2_AUTH["METADATA_AUTO_CONF_URL"],},]}
...
def _create_new_user(
    ...
    for g in AD_SUPERUSER_GROUPS:
        if g in user_group_names:
            user.is_superuser = True
    for g in AD_STAFF_GROUPS:
        if g in user_group_names:
            user.is_staff = True
    print(AD_SUPERUSER_GROUPS)
...
def acs(r):
    ...
    my_attrs = {
        x: "unknown"
        for x in ["user_email", "user_first_name", "user_last_name", "user_name"]
    }
    ...
    try:
        ...
        my_attrs["user_name"] = (
            my_attrs["user_first_name"][0] + my_attrs["user_last_name"].lower()
        )
    ...
    if "Groups" in user_identity:
        for group in user_identity["Groups"]:
            ...
            (ogrp,_) = Group.objects.get_or_create(name=group_name)
